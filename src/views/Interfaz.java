...
public class Interfaz extends JFrame{
    ...
    public void updateSaldoWallet(int value) {
        controlador.updateSaldoWallet(controlador.getClientes().get(panelClientes.getActiveUser()).getWallet().getSaldo()+value, controlador.getClientes().get(panelClientes.getActiveUser()).getWallet().getId());
    }

    public void updateRetirarSaldoWallet(int value) {
        controlador.updateSaldoWallet(controlador.getClientes().get(panelClientes.getActiveUser()).getWallet().getSaldo()-value, controlador.getClientes().get(panelClientes.getActiveUser()).getWallet().getId());        
    }
    ...
    public void insertTransaction(int type, int value) {
        if (panelClientes.getActiveUser()==-1) {
            ...
            if (type == 1){
                ...
                updateRetirarSaldoWallet(value);
            }
