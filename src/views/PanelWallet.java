...
public class PanelWallet extends JPanel implements ActionListener {
    ...
    public void retirarWallet() {
        if (getActiveWallet() == -1) {
            JOptionPane.showMessageDialog(interfaz, "Primero debes escoger un usuario de la lista.");
        }else{
            String valor = JOptionPane.showInputDialog(interfaz, "Digite la cantidad a retirar.");
            try {            
                int value = Integer.parseInt(valor);
                interfaz.insertTransaction(2,value);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals(btnCargar.getActionCommand())) {
            cargarWallet();
        }else if (e.getActionCommand().equals(btnRetirar.getActionCommand())) {
            retirarWallet();
        }
    }
